...
def create_window():
    sg.theme("Black")
    layout = [
        [sg.Push(), sg.Image('exit_15_15.png', pad=0, enable_events=True, key="-CLOSE-")],
        [sg.VPush()],
...
while True:
    ...
    if event == "-LAP-":
        window.extend_layout(window["-LAPS-"], [[sg.Text(lap_counter), sg.VSeparator(), sg.Text(elapsed_time)]])
        lap_counter += 1
